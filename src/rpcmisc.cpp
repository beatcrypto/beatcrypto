...
// Copyright (c) 2017-2018 The BeatCrypto developers
...
Value getinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total beatcrypto balance of the wallet (excluding zerocoins)\n"
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in beatcrypto/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in beatcrypto/kb\n"
    ...
    Object zbeatcryptoObj;
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zbeatcryptoObj.push_back(Pair(to_string(denom), ValueFromAmount(chainActive.Tip()->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zbeatcryptoObj.emplace_back(Pair("total", ValueFromAmount(chainActive.Tip()->GetZerocoinSupply())));
    obj.emplace_back(Pair("zBEATsupply", zbeatcryptoObj));
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"beatcryptoaddress\"\n"
            "\nReturn information about the given beatcrypto address.\n"
            "\nArguments:\n"
            "1. \"beatcryptoaddress\"     (string, required) The beatcrypto address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"beatcryptoaddress\", (string) The beatcrypto address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: BeatCrypto address and we have full public key:
...
Value createmultisig(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
                     ...
                     "2. \"keys\"       (string, required) A json array of keys which are beatcrypto addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"key\"    (string) beatcrypto address or hex-encoded public key\n"
                     "       ,...\n"
                     "     ]\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"beatcryptoaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"beatcryptoaddress\"  (string, required) The beatcrypto address to use for the signature.\n"
