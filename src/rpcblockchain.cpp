...
Object blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails = false)
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zbeatcryptoObj.push_back(Pair(to_string(denom), ValueFromAmount(blockindex->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zbeatcryptoObj.emplace_back(Pair("total", ValueFromAmount(blockindex->GetZerocoinSupply())));
    result.emplace_back(Pair("zBEATsupply", zbeatcryptoObj));
...
Value getrawmempool(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 1)
        throw runtime_error(
            ...
            "    \"fee\" : n,              (numeric) transaction fee in beatcrypto\n"
...
Value gettxout(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 3)
        throw runtime_error(
            ...
            "     \"addresses\" : [          (array of string) array of beatcrypto addresses\n"
