...
*Setup instructions for a gitian build of BeatCrypto using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the BeatCrypto
...
- [Building BeatCrypto](#building-beatcrypto)
...
Clone the git repositories for beatcrypto and gitian and then checkout the beatcrypto version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the beatcrypto repository
...
To build BeatCrypto (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the beatcrypto repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/beatcrypto/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/eastcoastcrypto/BeatCrypto
...
For example:
```bash
URL=https://github.com/eastcoastcrypto/BeatCrypto.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit beatcrypto=${COMMIT} --url beatcrypto=${URL} ../BeatCrypto/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit beatcrypto=${COMMIT} --url beatcrypto=${URL} ../BeatCrypto/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit beatcrypto=${COMMIT} --url beatcrypto=${URL} ../BeatCrypto/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/beatcrypto-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/beatcrypto-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/beatcrypto-build.assert
```
