...
Value obfuscation(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() == 0)
        throw runtime_error(
            "obfuscation <beatcryptoaddress> <amount>\n"
            "beatcryptoaddress, reset, or auto (AutoDenominate)"
    ...
    if (params.size() != 2)
        throw runtime_error(
            "obfuscation <beatcryptoaddress> <amount>\n"
            "beatcryptoaddress, denominate, or auto (AutoDenominate)"
            "<amount> is a real and will be rounded to the next 0.1" +
            HelpRequiringPassphrase());
...
Value getpoolinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "\nResult:\n"
            "{\n"
            "  \"current\": \"addr\",    (string) BeatCrypto address of current masternode\n"
...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (fHelp ||
        ...
        throw runtime_error(
            ...
            "  start        - Start masternode configured in beatcrypto.conf\n"
...
Value listmasternodes(const Array& params, bool fHelp)
{
    ...
    if (fHelp || (params.size() > 1))
        throw runtime_error(
            ...
            "    \"addr\": \"addr\",      (string) Masternode BeatCrypto address\n"
...
Value masternodeconnect(const Array& params, bool fHelp)
{
    if (fHelp || (params.size() != 1))
        throw runtime_error(
            ...
            "\nExamples:\n" +
            HelpExampleCli("masternodeconnect", "\"192.168.0.6:18898\"") + HelpExampleRpc("masternodeconnect", "\"192.168.0.6:18898\""));
...
Value getmasternodestatus (const Array& params, bool fHelp)
{
    if (fHelp || (params.size() != 0))
        throw runtime_error(
            ...
            "  \"addr\": \"xxxx\",        (string) BeatCrypto address for masternode payments\n"
            "  \"status\": \"xxxx\",      (string) Masternode status\n"
...
Value getmasternodewinners (const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw runtime_error(
            ...
            "    \"winner\": {\n"
            "      \"address\": \"xxxx\",    (string) BeatCrypto MN Address\n"
