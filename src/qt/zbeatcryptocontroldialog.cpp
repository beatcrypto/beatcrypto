...
#include "ui_zbeatcryptocontroldialog.h"
...
std::list<std::string> ZBeatCryptoControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZBeatCryptoControlDialog::listMints;

ZBeatCryptoControlDialog::ZBeatCryptoControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZBeatCryptoControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZBeatCryptoControlDialog::~ZBeatCryptoControlDialog()
{
    delete ui;
}

void ZBeatCryptoControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZBeatCryptoControlDialog::updateList()
{
...
void ZBeatCryptoControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZBeatCryptoControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZBeatCrypto_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZBeatCryptoControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZBeatCryptoControlDialog::GetSelectedMints()
{
...
void ZBeatCryptoControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
