...
This mechanism has been integrated into Bitcore-Node-BeatCrypto which allows for notification to be broadcast through Insight API in one of two ways:
* WebSocket: 
* API:  
...
By default, the BeatCrypto Core daemon will launch using the following constant:

```
static const int DEFAULT_SWIFTTX_DEPTH = 5;
```

This value can be overridden by passing the following argument to the BeatCrypto Core daemon:
