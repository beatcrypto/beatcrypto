...
#include "zbeatcryptocontroldialog.h"
#include "spork.h"
...
void PrivacyDialog::on_pushButtonZBeatCryptoControl_clicked()
{
    ZBeatCryptoControlDialog* zBeatCryptoControl = new ZBeatCryptoControlDialog(this);
    zBeatCryptoControl->setModel(walletModel);
    zBeatCryptoControl->exec();
}

void PrivacyDialog::setZBeatCryptoControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzBeatCryptoSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzBEAT()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid BeatCrypto Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    // use mints from zBeatCrypto selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZBeatCryptoControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZBeatCryptoControlDialog::GetSelectedMints();
    }
    ...
    // Clear zbeatcrypto selector in case it was used
    ZBeatCryptoControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zBeatCrypto Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " BeatCrypto, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zBeatCrypto Mint");
