...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a BeatCrypto address (e.g. %1)").arg("PCYiHgGJJ6xGHqivmdZrYjRnhaYf6AJ2Mp"));
#endif
...
{
    // return if URI is not valid or is no BeatCrypto: URI
...
{
    // Convert beatcrypto:// to beatcrypto:
    //
    //    Cannot handle this later, because beatcrypto:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open beatcrypto.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "BeatCrypto.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for BeatCrypto.lnk
...
{
    return GetAutostartDir() / "beatcrypto.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a beatcrypto.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=BeatCrypto\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the beatcrypto app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add beatcrypto app to startup item list
